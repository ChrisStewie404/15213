...
word e_valA = [
	(E_icode == IRMMOVQ || E_icode == IPUSHQ) && M_dstM == E_srcA : m_valM;
	1 : E_valA;  
	# Use valA from stage pipe register 
];
...
bool F_stall =
	...
	E_icode in { IMRMOVQ, IPOPQ } &&( D_icode == IOPQ &&
	 E_dstM in { d_srcA, d_srcB } || 
	 D_icode == IIADDQ && E_dstM == d_srcB ||
	 D_icode == IRMMOVQ && E_dstM == d_srcB || 
	 D_icode == IMRMOVQ && E_dstM == d_srcA ||
	 D_icode == IRET && E_dstM == RRSP	||
	 D_icode == ICALL && E_dstM == RRSP) ||
